...
function drawPost(canvas: HTMLCanvasElement, deal: FlightDeal, destinationPhoto?: HTMLImageElement) {
  ...
  drawLocationPin(context, 67, 225);
  context.fillStyle = '#202a31';
  fittedFont(context, destinationName, 118, 62, 865);
  context.fillText(destinationName, 153, 307);
  context.font = '800 62px Arial';
  context.fillStyle = '#f25f45';
  context.fillText(`${deal.origin}  →  ${deal.destinationAirport}`, 68, 384);

  context.fillStyle = '#ffffff';
  context.beginPath();
  context.roundRect(65, 424, 950, 520, 34);
  context.fill();
  context.fillStyle = '#202a31';
  context.font = '700 32px Arial';
  context.fillText('ROUND TRIP FROM', 112, 504);
  context.font = '800 236px Arial';
  context.fillText(`$${deal.price}`, 95, 734);
  context.fillStyle = '#2b9a71';
  context.font = '800 39px Arial';
  context.fillText(`${deal.percentBelowTypical}% BELOW TYPICAL`, 112, 812);
  context.strokeStyle = '#e7e2da';
  context.lineWidth = 2;
  context.beginPath();
  context.moveTo(112, 854);
  context.lineTo(968, 854);
  context.stroke();
  context.fillStyle = '#5f686c';
  context.font = '700 25px Arial';
  context.fillText(`${deal.nonstop ? 'NONSTOP' : '1 STOP'}  ·  ${deal.airline.toUpperCase()}`, 112, 904);
  ...
  drawCalendar(context, 68, 970);
  fittedFont(context, outboundDate, 58, 42, 480);
  context.fillText(outboundDate, 140, 1025);
  fittedFont(context, returnDate, 58, 42, 480);
  context.fillText(returnDate, 140, 1095);
  context.font = '500 22px Arial';
  context.fillStyle = '#717a7d';
  context.fillText('Prices can change anytime. Verify before booking.', 68, 1178);
